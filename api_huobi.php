			...
			$trades .= "\"trades\":[";
				...
				$k=0;
					if ($result['data'][$i]['type'] != 'submit-cancel') {
						...
						$trades .= "\"qty\":".$result['data'][$i]['amount'].",";
...
<script type=\"text/javascript\" src=\"https://www.gstatic.com/charts/loader.js\"></script>
  ...
  function drawChart() {
	...
	while ($fcontents['data'][$i]) {
		$lasttime = getdate($fcontents['data'][$i]['id']/1000);
		$tcandle[$j] =  date("H:i", mktime($lasttime["hours"], $lasttime["minutes"], 0, $lasttime["mon"], $lasttime["mday"], $lasttime["year"]));
		$lowcandle[$j] = $fcontents['data'][$i]['low'];
		$opencandle[$j] = $fcontents['data'][$i]['open'];
		$closecandle[$j] = $fcontents['data'][$i]['close'];
		$highcandle[$j] = $fcontents['data'][$i]['high'];
		$volumecandle[$j] = $fcontents['data'][$i]['vol'];
